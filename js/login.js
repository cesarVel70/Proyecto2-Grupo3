...
form.addEventListener("submit", function (event) {
  ...
  if (correo === "admin@admin.com" && clave === "admin") {
    ...
    return showAlert(
      "Usuario no registrado y/o datos mal puestos",
      "alert-danger"
    );
  } else {
    setRol("usuario");
...
CapturarRecuperacion.addEventListener("click", function () {
  let correoGuardado = prompt(
    "Para recuperar la contraseña, debe ingresar el correo con el cual se registro"
  );

  let usuarios = JSON.parse(localStorage.getItem("users")) || [];
  let buscarCorreo = usuarios.find(
    (usuario) => usuario.correo === correoGuardado
  );
